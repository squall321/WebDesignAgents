# 위젯 수용 템플릿 3종 preview 실렌더 검증 — svg 마운트·콘솔 에러 0·씬 DOM 실존·최소 폰트 24px·오버플로 0·frame-match 실측
...
DATA = ["d-matrix", "d-media", "d-multi"]
# typical 은 대표 구성(실물 유도), max 는 스키마 상한 — 상한이 격자/그리드 존 안에 드는지가 실측 역산의 증명
FIXTURES = ["typical", "max"]
CASES = [(n, f) for n in DATA for f in FIXTURES]
...
# d-media 이미지 실측 — img 로드 성공(naturalWidth)과 표시 상자
_IMG_JS = """
(svgSel) => {
  const svg = document.querySelector(svgSel);
...
def measure_previews(write_snapshots: bool = False) -> dict:
    """3종 preview × (typical, max) 를 1920×1080 원척으로 실렌더하고 측정치를 모은다.
...
def test_media_images_load_and_fill(rendered, fixture):
    """d-media 이미지가 실제 로드되고(naturalWidth>0) 표시 상자가 무대 안에 있다 — 레터박스 금지의 실측."""
    images = rendered[f"d-media:{fixture}"]["images"]
    expect = 2 if fixture == "typical" else 3
    assert len(images) == expect, f"d-media/{fixture}: 이미지 {len(images)}장 (기대 {expect})"
    for im in images:
...
if __name__ == "__main__":
    # 스냅샷 갱신 진입점 — uv run python tests/test_templates_data_render.py
