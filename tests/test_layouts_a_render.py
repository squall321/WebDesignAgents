# 발표 레이아웃 템플릿 4종 preview 실렌더 검증 — svg 마운트·콘솔 에러 0·씬 DOM 실존·최소 폰트 24px·오버플로 0·frame-match 실측
...
# 콘텐츠 하단 한계 — 푸터(bottom 56 + padTop 22 + 캡션 1행 29) 위 18px, omx-layouts-a.jsx botLimit 와 동일
CONTENT_BOTTOM = 955.0

LAYOUTS = ["l-split", "l-list", "l-tree", "l-quote"]
# l-split 은 근거 슬롯 4종(table/bars/image/note)을 모두 실렌더해야 밀도 검증이 끝난다
FIXTURES = {
    "l-split": ["min", "typical", "max", "image", "note"],
    "l-list": ["min", "typical", "max"],
    # deep = 가로 상한(4가지)과 별개의 세로 상한(2가지 × 리프 4개)
    "l-tree": ["min", "typical", "max", "deep"],
    "l-quote": ["min", "typical", "max"],
}
CASES = [(n, f) for n in LAYOUTS for f in FIXTURES[n]]
...
# l-split image 슬롯 실측 — img 로드 성공(naturalWidth)과 표시 상자
_IMG_JS = """
(svgSel) => {
  const svg = document.querySelector(svgSel);
  const base = svg.getBoundingClientRect();
  return [...svg.querySelectorAll('img')].map((el) => {
...
def measure_previews(write_snapshots: bool = False) -> dict:
    """4종 preview × 픽스처를 1920×1080 원척으로 실렌더하고 측정치를 모은다.
    ...
    with StaticServer(ROOT) as srv, sync_playwright() as pw:
        ...
        for name, fixture in CASES:
            ...
            images = page.evaluate(_IMG_JS, EXPORTABLE_SVG)
...
def test_scene_dom_exists(rendered, name, fixture):
    ...
    assert layer["nodes"] >= 5, f"{name}/{fixture}: 씬 DOM 노드가 {layer['nodes']}개뿐"
...
@pytest.mark.parametrize("name,fixture", CASES)
def test_content_stays_above_footer(rendered, name, fixture):
    """밀도를 높인 결과가 푸터를 침범하지 않는다 — 콘텐츠 텍스트 하단 ≤ 955px.

    푸터 자체(브랜드·쪽번호)는 대상에서 제외한다.
    """
    r = rendered[f"{name}:{fixture}"]
    footer_texts = set()
    fr = (json.loads((TPL_DIR / name / "fixtures" / f"{fixture}.json").read_text(encoding="utf-8"))
          .get("frame") or {})
    if fr:
        footer_texts.add(fr.get("brand", ""))
        footer_texts.add(f"{fr.get('idx', '')} / {fr.get('total', '')}")
    bad = []
    for it in r["scan"]["items"]:
        if not it["hasText"] or not it["textRect"] or it["text"] in footer_texts:
            continue
        bottom = it["textRect"]["y"] + it["textRect"]["h"]
        if bottom > CONTENT_BOTTOM + 1.0:
            bad.append((it["text"][:24], round(bottom, 1)))
    assert not bad, f"{name}/{fixture}: 콘텐츠가 푸터 영역({CONTENT_BOTTOM:.0f}px)을 침범 — {bad}"


@pytest.mark.parametrize("fixture", FIXTURES["l-split"])
def test_split_bullets_clear_the_pinned_conclusion(rendered, fixture):
    """설명 열 하단 고정 소결론과 불릿이 겹치지 않는다 — 밀도 상한의 실제 증명."""
    data = json.loads(
        (TPL_DIR / "l-split" / "fixtures" / f"{fixture}.json").read_text(encoding="utf-8")
    )
    conc = data.get("conclusion") or {}
    if not conc:
        pytest.skip("소결론 없는 픽스처")
    bullet_texts = {b["text"] for b in data["bullets"]}
    conc_text = f"{conc.get('pre', '')}{conc.get('post', '')}".replace("  ", " ").strip()
    bullet_bottom = conc_top = None
    for it in rendered[f"l-split:{fixture}"]["scan"]["items"]:
        if not it["hasText"] or not it["textRect"]:
            continue
        g = it["textRect"]
        if it["text"] in bullet_texts:
            bullet_bottom = max(bullet_bottom or 0.0, g["y"] + g["h"])
        elif conc_text and it["text"].replace("  ", " ").strip() == conc_text:
            conc_top = g["y"] if conc_top is None else min(conc_top, g["y"])
    assert bullet_bottom is not None, "불릿 텍스트를 스캔에서 찾지 못했다"
    assert conc_top is not None, f"소결론 텍스트({conc_text!r})를 스캔에서 찾지 못했다"
    assert bullet_bottom < conc_top, (
        f"l-split/{fixture}: 불릿 하단 {bullet_bottom:.0f}px 가 소결론 상단 {conc_top:.0f}px 를 침범"
    )


def test_split_image_slot_loads(rendered):
    """l-split image 슬롯의 도판이 실제 로드되고(naturalWidth>0) 무대 안에 놓인다."""
    images = rendered["l-split:image"]["images"]
    assert len(images) == 1, f"l-split/image: 이미지 {len(images)}장 (기대 1)"
    im = images[0]
    assert im["naturalW"] > 0 and im["naturalH"] > 0, f"이미지 로드 실패 — {im['src']}"
    assert im["alt"], f"alt 누락 — {im['src']}"
    assert im["w"] > 0 and im["h"] > 0
    assert im["x"] >= -1 and im["y"] >= -1
    assert im["x"] + im["w"] <= STAGE_W + 1 and im["y"] + im["h"] <= STAGE_H + 1
...
if __name__ == "__main__":
    # 스냅샷 갱신 진입점 — uv run python tests/test_layouts_a_render.py
